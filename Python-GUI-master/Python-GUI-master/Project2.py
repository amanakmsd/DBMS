...
##########################################################################################
    ...
    def file_save(self):
        ...
        for player in self.items:  #################################################################################################issue#########
        ...
        for i in range(len(List)):
            curser.execute("INSERT INTO team('NAMES','PLAYERS','SCORES') VALUES ('%s','%s','%d');"%(teamname,List[i][0],int(List[i][1])))
    ...
    def combo2(self):                   #################################################################################################issue###########
       ...
       # mymatch=sqlite3.connect("match.db")
        #cur=mymatch.cursor()
        #match = self.eval_screen.Selectmatch.currentText()
        cu.execute("SELECT SCORES from team WHERE NAMES='"+team+"';")  ######################################3
        score=cu.fetchall()   ##without scores drctly accesspoints from match no. by making the player name as primary key and use procedure 
        self.teamscore=[]
